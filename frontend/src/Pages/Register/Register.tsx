...
import { User, Mail, Lock, ShieldCheck, Image as ImageIcon, ArrowLeft, X, CheckCircle2 } from 'lucide-react';
              ...
              {step === 1 ? (
                ...
                >
                  <h1 className="text-4xl md:text-5xl font-bold leading-tight mb-6">
                    {t('register_page.welcome')} <span className="text-blue-200">SmartShop</span>
                  </h1>
                  ...
                  <div className="flex items-start gap-4 p-4 bg-white/10 rounded-2xl backdrop-blur-md border border-white/20">
                    ...
                    <div className={isRtl ? 'text-right' : 'text-left'}>
                      <h3 className="font-semibold text-white mb-1">{t('register_page.email_verify')}</h3>
                ...
                >
                  <h1 className="text-4xl md:text-5xl font-bold leading-tight mb-6">
                    {t('register_page.check_inbox')} <span className="text-blue-200">{t('register_page.inbox')}</span>
                  </h1>
                  <p className="text-lg text-blue-100 mb-8 font-light">
                    {t('register_page.check_inbox_desc')} <span className="font-bold text-white underline underline-offset-4">{formData.email}</span>.
                  </p>
                  <div className="flex items-start gap-4 p-4 bg-white/10 rounded-2xl backdrop-blur-md border border-white/20">
            ...
            {step === 1 ? (
              ...
              >
                <div className={`mb-10 ${isRtl ? 'text-right' : 'text-left'}`}>
                  <h2 className="text-3xl font-bold text-slate-900 mb-2">{t('register_page.create_account')}</h2>
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className={`w-full px-4 py-4 rounded-xl border transition-all outline-none ${
                        errors.name ? 'border-red-500 bg-red-50' : 'border-slate-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 bg-slate-50'
                      }`}
                      placeholder="John Doe"
                      value={formData.name}
                      onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                    />
                    {errors.name && <p className={`text-xs text-red-500 font-medium ${isRtl ? 'text-right' : 'text-left'} pl-1 pt-1`}>{errors.name}</p>}
                  </div>
                  ...
                  <div className={`pt-4 flex flex-col sm:flex-row gap-4 ${isRtl ? 'sm:flex-row-reverse' : ''}`}>
                    <button
                      onClick={handleNext}
              ...
              >
                <div className={`mb-8 ${isRtl ? 'text-right' : 'text-left'}`}>
                  <h2 className="text-3xl font-bold text-slate-900 mb-2">{t('register_page.final_step')}</h2>
                  <p className="text-slate-500">{t('register_page.complete_profile')}</p>
                </div>

                <form onSubmit={handleSubmit} className="space-y-5">
                  <div className={`grid grid-cols-2 gap-4 ${isRtl ? 'text-right' : 'text-left'}`}>
                    <div className="space-y-1">
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">{t('register_page.name_label')}</label>
                      <div className="px-4 py-3 bg-slate-50 border border-slate-200 rounded-lg text-slate-500 text-sm font-medium">
                        {formData.name}
                      </div>
                    </div>
                    <div className="space-y-1">
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">{t('register_page.email_label')}</label>
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className={`w-full px-4 py-3 rounded-xl border text-center text-2xl tracking-[0.5em] font-bold outline-none transition-all ${
                        errors.otp ? 'border-red-500 bg-red-50' : 'border-slate-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 bg-slate-50'
                      }`}
                      placeholder="000000"
                      value={formData.otp}
                      onChange={(e) => setFormData({ ...formData, otp: e.target.value.replace(/\D/g, '') })}
                    />
                    {errors.otp && <p className={`text-xs text-red-500 font-medium ${isRtl ? 'text-right' : 'text-left'} pl-1 pt-1`}>{errors.otp}</p>}
                  </div>
                  ...
                  {/* Password Input */}
                    ...
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        dir="ltr"
                        className={`w-full px-4 py-3 rounded-xl border outline-none transition-all ${isRtl ? 'text-right' : 'text-left'} ${
                          errors.confirmPassword ? 'border-red-500 bg-red-50' : 'border-slate-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 bg-slate-50'
                        }`}
                        placeholder="••••••••"
                        value={formData.confirmPassword}
                        onChange={(e) => setFormData({ ...formData, confirmPassword: e.target.value })}
                      />
                      {errors.confirmPassword && <p className={`text-xs text-red-500 font-medium ${isRtl ? 'text-right' : 'text-left'} pl-1 pt-1`}>{errors.confirmPassword}</p>}
                    </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className={`flex items-center gap-4 ${isRtl ? 'flex-row-reverse' : ''}`}>
                      ...
                      <div className={`flex-1 ${isRtl ? 'text-right' : 'text-left'}`}>
                        <p className="text-xs text-slate-500">{t('register_page.img_specs')}</p>
                        {formData.image && (
                          <div className={`flex items-center gap-1 text-xs text-green-600 font-bold mt-1 ${isRtl ? 'flex-row-reverse' : ''}`}>
                            <CheckCircle2 size={12} />
                            <span className="truncate">{formData.image.name}</span>
                          </div>
                        )}
                        {errors.image && <p className="text-xs text-red-500 font-medium mt-1">{errors.image}</p>}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="pt-4 flex flex-col gap-3">
                    ...
                    >
                      {isRtl ? <ArrowLeft size={16} className="rotate-180" /> : <ArrowLeft size={18} />}
